...
func TestCapability_HostCallRetriesConfiguredOperationPrefix(t *testing.T) {
	host := newMockHost()
	host.callDeadlineFailures = 1
	host.callResult = api.CallResult{Value: map[string]any{"ok": true}}
...
func TestCapability_HostCallExcludesAllowlistedOperationFromRetry(t *testing.T) {
	host := newMockHost()
	host.callDeadlineFailures = 1
	host.callResult = api.CallResult{Value: map[string]any{"ok": true}}
	...
	vm, err := New(mod, Options{
		...
		RetryBackoff:         time.Millisecond,
		RetryCallOps:         []string{"fs.read_file"},
		RetryExcludedCallOps: []string{"fs.read_file"},
	})
	if err != nil {
		t.Fatalf("New VM failed: %v", err)
	}
...
func TestCapability_HostCallDisableRetryOverridesGlobalRetryPolicy(t *testing.T) {
	host := newMockHost()
	host.callDeadlineFailures = 1
	host.callResult = api.CallResult{Value: map[string]any{"ok": true}}
...
func TestCapability_HostCallCanDisableOnlyCallRetry(t *testing.T) {
	host := newMockHost()
	host.callDeadlineFailures = 1
	host.callResult = api.CallResult{Value: map[string]any{"ok": true}}
...
func TestCapability_HostCallKeepsRetryWhenPollRetryIsDisabled(t *testing.T) {
	host := newMockHost()
	host.callDeadlineFailures = 1
	host.callResult = api.CallResult{Value: map[string]any{"ok": true}}
	...
	if len(host.callLog) != 2 {
		t.Fatalf("expected host.call retry to remain enabled, got %d attempts", len(host.callLog))
	}
}

func TestCapability_HostCallKeepsRetryWhenWaitRetryIsDisabled(t *testing.T) {
	host := newMockHost()
	host.callDeadlineFailures = 1
	host.callResult = api.CallResult{Value: map[string]any{"ok": true}}
...
func TestCapability_HostCallExclusionOverridesRetryOperationPrefix(t *testing.T) {
	host := newMockHost()
	host.callDeadlineFailures = 1
	host.callResult = api.CallResult{Value: map[string]any{"ok": true}}

...
func TestSerializeNilUserFunction(t *testing.T) {
	chunk := &bc.Chunk{
		Code: []bc.Instruction{
			{Op: bc.OpConstant, A: 0, B: 0},
			{Op: bc.OpReturn, A: 0, B: 0},
		},
		Constants: []any{(*rttypes.UserFunction)(nil)},
	}
	data, err := Serialize(chunk)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
...
func TestSerializeUserFunctionInvalidChunk(t *testing.T) {
	chunk := &bc.Chunk{
		Code: []bc.Instruction{
			{Op: bc.OpClosure, A: 0, B: 0},
			{Op: bc.OpReturn, A: 0, B: 0},
		...
		Constants: []any{
			&rttypes.UserFunction{Name: "f", Chunk: "not a chunk"},
		},
	}
	_, err := Serialize(chunk)
	if err == nil {
...
func TestSerializeFunctionEnvError(t *testing.T) {
	chunk := &bc.Chunk{
		Code: []bc.Instruction{
			{Op: bc.OpClosure, A: 0, B: 0},
			{Op: bc.OpReturn, A: 0, B: 0},
	...
	_, err := Serialize(chunk)
	if err == nil {
		t.Fatal("Serialize expected env error, got nil")
	}
}
